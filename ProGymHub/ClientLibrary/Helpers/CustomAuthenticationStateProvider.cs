...
namespace ClientLibrary.Helpers
{
    public class CustomAuthenticationStateProvider(LocalStorageService localStorageService) : AuthenticationStateProvider
    {
        ...
        public async Task UpdateAuthenticationState(UserSession userSesion)
        {
            var claimsPrincipal = new ClaimsPrincipal();
            if(userSesion.Token != null || userSesion.RefreshToken != null)
            {
                var serializeSession = Serializations.SerializeObj(userSesion);
                await localStorageService.SetToken(serializeSession);
                var getUserClaims = DecryptToken(userSesion.Token!);
